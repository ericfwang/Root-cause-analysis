...
by_region <- import %>% 
  ...
  regional_plot <- line_plot(by_region, "occupancy", "city", "Occupancy Rate by Region", "Occupancy Rate", "City")
...
counterfactual <- import %>% 
  ...
  counterfactual_plot <- line_plot(counterfactual, "occupancy", "type", "Occupancy Rate With/Without Los Angeles", "Occupancy Rate", "")
...
los_angeles_apartment <- import %>% 
  ...
  los_angeles_apartments_plot <- line_plot(los_angeles_apartment, "rate", "type", "Contribution to Bookings and Availability in Los Angeles by Space Type", "Monthly Share", "Metric")
